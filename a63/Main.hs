...
main = do
  [n, m] <- getLineIntList
  edges <- replicateM m getLineIntList

  let edgeInputs = concatMap (\[!a, !b] -> [(a, b), (b, a)]) edges
      links = accumArray (flip IS.insert) IS.empty (1, n) edgeInputs

  -- REMARK: We run the `dfs` just once and use the result throughout all the queries!
  let costs = dfs 1 links

  let results = map (\i -> IM.findWithDefault (-1) i costs) [1 .. n]
      s = intersperse (BSB.charUtf8 '\n') $ map BSB.intDec results
  BSB.hPutBuilder stdout (mconcat @BSB.Builder s)

dfs :: Int -> Array Int IS.IntSet -> IM.IntMap Int
dfs from links = loop initialCosts (IS.singleton from) 0
  where
    initialCosts = IM.singleton from 0
    loop :: IM.IntMap Int -> IS.IntSet -> Int -> IM.IntMap Int
    loop costs verts cost
      | IS.null verts = costs
      | otherwise =
        -- NOTE: all the vertices are not explored yet
        let newCosts = IM.union costs (IM.fromSet (const cost) verts)
            newVerts = IS.fromList . filter isUnexplored . concatMap (IS.toList . (links !)) $ IS.elems verts
         in loop newCosts newVerts (succ cost)
      where
        isUnexplored i = IM.notMember i costs
