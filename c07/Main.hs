...
main = do
  [n] <- getLineIntList
  costs <- VU.modify VAI.sort <$> getLineIntVec
  [q] <- getLineIntList
  haves <- concat <$> replicateM q getLineIntList

  let sums = VU.scanl1' (+) costs

  forM_ haves $ \have -> do
    print $ case fst $ bsearch (0, VU.length sums - 1) (\i -> sums VU.! i <= have) of
      Just i -> succ i
      Nothing -> 0
