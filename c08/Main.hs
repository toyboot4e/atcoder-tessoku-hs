...
solve input = unwrap . mapMaybe inner $ [[a, b, c, d] | a <- ['0' .. '9'], b <- ['0' .. '9'], c <- ['0' .. '9'], d <- ['0' .. '9']]
  where
    unwrap [a] = Just a
    unwrap _ = Nothing

    inner :: String -> Maybe String
    inner str =
      if all (uncurry (match str)) input
        then Just str
        else Nothing

    match :: String -> String -> Int -> Bool
    match str s t =
      let neq = length $ filter id $ zipWith (==) str s
       in case (t, neq) of
            (1, 4) -> True
            (2, 3) -> True
            (3, 4) -> False
...
main = do
  [n] <- getLineIntList
  let localParse (a : b : c : d : _ : f) = ([a, b, c, d], read @Int f)
  input <- replicateM n (localParse <$> getLine)

  putStrLn $ fromMaybe "Can't Solve" (solve input)
