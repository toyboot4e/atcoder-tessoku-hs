...
main :: IO ()
main = do
  xxx@(nVerts, _nEdges) <- getTuple2
  graph <- getWGraph xxx

  let d1 = dijkstra f IM.empty graph 1
      dn = dijkstra f IM.empty graph nVerts
      f :: IM.IntMap Int -> H.Entry Int Int -> IM.IntMap Int
      f im (H.Entry cost vertex) = IM.insert vertex cost im

  let d = d1 IM.! nVerts

  let result = length $ filter g [1 .. nVerts]
      g v = d1 IM.! v + dn IM.! v == d
