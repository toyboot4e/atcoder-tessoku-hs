...
main = do
  ...
  l2p <- VUM.replicate (n + 1) (maxBound @ Int)
  VUM.write l2p 0 0

  let f = fst <$> foldM step s0 input
      s0 = (0 :: Int, IM.empty @Int)
      step :: (Int, IM.IntMap Int) -> Int -> IO (Int, IM.IntMap Int)
      step (maxLen, p2l) p = do
        newLen <- do
          l <- fromMaybe 0 . fst <$> bsearchM (0, maxLen) (fmap (<= p) . VUM.read l2p)
          p' <- VUM.read l2p l
          when (p' > p) $ error "unreachable"
          return $ if p == p' then l else succ l
        VUM.modify l2p (min p) newLen
        let p2l' = IM.insertWith max p newLen p2l
        return (max maxLen newLen, p2l')
