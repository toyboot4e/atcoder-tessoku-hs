...
main = do
  [n] <- getLineIntList
  [k] <- getLineIntList
  input <- VU.fromList <$> replicateM n (second (+ k) . tuple2 <$> getLineIntList)

  let greedyL = VU.fromList . reverse . snd $ VU.foldl' f (-1, []) inputL
      inputL = VU.modify (VAI.sortBy (comparing snd)) input
      f :: (Int, [Int]) -> (Int, Int) -> (Int, [Int])
      f (lastR, acc) (l, r)
        | lastR <= l = (r, r : acc)
        | otherwise = (lastR, acc)

  let greedyR = VU.fromList . snd $ VU.foldl' g (86400 + 1, []) inputR
      inputR = VU.modify (VAI.sortBy (comparing $ negate . fst)) $ VU.reverse input
      g :: (Int, [Int]) -> (Int, Int) -> (Int, [Int])
      g (lastL, acc) (l, r)
        | r <= lastL = (l, l : acc)
        | otherwise = (lastL, acc)

  let findL upper = maybe 0 succ . fst $ bsearch (0, pred $ VU.length greedyL) $ \i -> greedyL VU.! i <= upper
  let findR lower = maybe 0 (\r -> VU.length greedyR - r) . snd $ bsearch (0, pred $ VU.length greedyR) $ \i -> greedyR VU.! i < lower

  let !_ = traceShow (inputL) ()
  let !_ = traceShow (greedyL) ()
  let !_ = traceShow (inputR) ()
  let !_ = traceShow (greedyR) ()

  VU.forM_ input \(l, r) -> do
    let nl = findL l
    let nr = findR r
    let !_ = traceShow ((l, r), nl, nr) ()
    print $ nl + 1 + nr
