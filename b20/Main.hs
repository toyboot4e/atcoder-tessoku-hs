...
main = do
  ...
  let bounds_ = ((0, 0), (BS.length s1, BS.length s2))

  -- dp: (nReadS1, nReadS2) -> cost
  let dp = runSTUArray $ do
        arr <- newArray bounds_ (maxBound @Int)
        writeArray arr (0, 0) 0

        -- I don't really understand the DP (T^T)
        forM_ (range bounds_) $ \(i1, i2) -> do
          -- FIXME: this ridiculous branch
          if (i1, i2) == (0, 0)
            then return ()
            else do
              v1 <-
                -- on discard s1_i1
                if inRange bounds_ (i1 - 1, i2)
                  then succ <$> readArray arr (i1 - 1, i2)
                  else return (maxBound @Int)
              v2 <-
                -- on insert s1_i1
                if inRange bounds_ (i1, i2 - 1)
                  then succ <$> readArray arr (i1, i2 - 1)
                  else return (maxBound @Int)
              v3 <-
                -- on replace or no change
                if inRange bounds_ (i1 - 1, i2 - 1)
                  then
                    ((+) if BS.index s1 (i1 - 1) == BS.index s2 (i2 - 1) then 0 else 1)
                      <$> readArray arr (i1 - 1, i2 - 1)
                  else return (maxBound @Int)
              -- let !_ = traceShow ((i1, i2), (v1, v2, v3)) ()
              writeArray arr (i1, i2) (minimum [v1, v2, v3])

        return arr

  -- let !_ = traceMat2D dp (0, BS.length s2) (0, BS.length s1)
  print $ dp ! snd bounds_
