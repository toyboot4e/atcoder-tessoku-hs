...
solve :: Int -> Int -> Int -> VU.Vector (Int, Int) -> Int
solve n 0 nz _ =
      let zz = nz * pred nz `div` 2
          zx = nz * (n - nz)
       in zz + zx
solve _ p _ xs = (`div` 2) . VU.sum $ VU.map f xs
  where
    f (d, count) =

      case ff (divModF p d modulus) of
        Nothing -> 0
        Just i ->
          let (d', count') = xs VU.! i
              -- !_ = traceShow (d, count, d', count', if d == d' then count * pred count' else count * count') ()
           in if d == d'
                then count * pred count'
                else count * count'

    -- Returns index
    ff :: Int -> Maybe Int
    ff x =
      case fst $ bsearch (0, pred $ VU.length xs) $ \i -> fst (xs VU.! i) <= x of
        Just i | x == fst (xs VU.! i) -> Just i
        _ -> Nothing
...
main = do
  ...
  -- [(x, count)]
  xs_ <- map (\g -> (head g, length g)) . group . sort . map (`mod` modulus) <$> getLineIntList

  let (nz, xs') =
        if fst (head xs_) == 0
          then (snd (head xs_), tail xs_)
          else (0, xs_)

  let xs = VU.fromList xs'

  print $ solve n p nz xs
