...
main = do
  [h, w] <- getLineIntList
  input <- map (== '#') . concat <$> replicateM h getLine
  let isBlock = listArray @UArray ((0, 0), (pred h, pred w)) input

  let flow = runSTUArray $ do
        arr <- newArray ((0, 0), (h - 1, w - 1)) (0 :: Int)
        writeArray arr (0, 0) 1

        forM_ [0 .. pred h] $ \y -> do
          forM_ [0 .. pred w] $ \x -> do
            unless ((y, x) == (0, 0) || isBlock ! (y, x)) $ do
              v1 <- if x > 0 then readArray arr (y, x - 1) else return 0
              v2 <- if y > 0 then readArray arr (y - 1, x) else return 0
              writeArray arr (y, x) (v1 + v2)

        return arr

  let !_ = traceMat2D flow (0, h - 1) (0, w - 1)
  print $ flow ! (h - 1, w - 1)
