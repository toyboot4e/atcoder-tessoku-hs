...
toBitMask :: [Int] -> Int
toBitMask bits = sum $ zipWith shift (reverse bits) [0 ..]

main :: IO ()
main = do
  [nItems, mTickets] <- getLineIntList

  -- -- failure: the accumulated list is not sorted by the bitmask, so the deduplication is not working
  -- masks <- VU.fromList . map toBitMask <$> replicateM mTickets getLineIntList
  -- let dp = VU.foldl' step [(0, 0)] masks
  --     step acc x =
  --       -- let !_ = traceShow (map (\(n, bits) -> (n + (1 :: Int), bits .|. x)) acc) () in
  --       merge acc $ map (\(n, bits) -> (n + (1 :: Int), bits .|. x)) acc
  --     merge [] ys = ys
  --     merge xs [] = xs
  --     merge xs@((xn, xb) : xrest) ys@((yn, yb) : yrest) = case compare xb yb of
  --       EQ -> (min xn yn, xb) : merge xrest yrest
  --       LT -> (xn, xb) : merge xrest ys
  --       GT -> (yn, yb) : merge xs yrest

  masks <- map toBitMask <$> replicateM mTickets getLineIntList
  let dp = foldl' step (IM.singleton 0 (0 :: Int)) masks
      -- acc: mask -> cost
      step acc mask =
        IM.unionWith min acc (IM.fromList . dedup . map (bimap (mask .|.) (1 +)) $ IM.toList acc)
      dedup :: [(Int, Int)] -> [(Int, Int)]
      dedup xs =
        map (minimumBy (comparing snd)) . groupBy (\a b -> fst a == fst b) . sortBy (comparing fst) $ xs

  let !_ = traceShow dp ()
  print $ fromMaybe (-1) $ IM.lookup (bit nItems - 1) dp
