...
main :: IO ()
main = do
  [nParties, nTickets] <- getLineIntList
  parties <- VU.map intToDouble <$> getLineIntVec

  -- TODO: inject completion predicate
  let border = fromJust . fst $ bsearchF64 (1.0, fromInteger @Double (10 ^ 9 - 1)) 0.0000005 f
      -- be careful of the direction of the iniquality
      f d = VU.sum (VU.map (floor @_ @Int . (/ d)) parties) >= nTickets

  -- let !_ = traceShow (border, parties, VU.map (`div` border) parties) ()
  putStrLn $ unwords $ map show $ VU.toList $ VU.map (floor @_ @Int . (/ border)) parties
