...
main = do
  ...
  let powers = VU.fromList $ scanl' (\acc _ -> acc * acc `mod` unit) a [1 .. 63]
  -- let !_ = traceShow powers ()

  let result = foldl' step 1 [0 .. 63]
      step acc nBit =
        if testBit b nBit
          then
              -- let !_ = traceShow (nBit, powers VU.! nBit) () in
              (acc * powers VU.! nBit) `mod` unit
          else acc
