...
main = do
  [n, k] <- getLineIntList
  as <- sort <$> getLineIntList
  bs <- sort <$> getLineIntList
  cs <- sort <$> getLineIntList
  ds <- VU.fromList . sort <$> getLineIntList
        ...
        not . null $
          [ True
            | a <- as,
              ...
              let ok = fst $ bsearch (0, n - 1) (\i -> y + ds VU.! i <= k),
              isJust ok,
              y + ds VU.! fromJust ok == k
          ]
