...
main = do
  ...
  -- REMARK: uses 0-based index: i -> (constraint, score)
  scores <- VU.replicateM n ((\[a, b] -> (a, b)) <$> getLineIntList)

  -- let !_ = traceShow n ()

  let dp = runSTUArray $ do
        -- REMARK: uses 1-based index
        arr <- newArray ((1, 1), (n, n)) 0
        -- writeArray arr (1, n) 0
        ...
        forM_ [2 .. n] $ \nDiag -> do
          forM_ [1 .. nDiag] $ \l -> do
            ...
            let bonus i =
                  let (constraint, score) = scores VU.! (i - 1)
                   in if inRange (l, r) constraint then score else 0

            v <-
              if
                  | l == 1 -> do
                    v <- readArray arr (l, r + 1)
                    return $ v + bonus (r + 1)
                  | r == n -> do
                    v <- readArray arr (l - 1, r)
                    return $ v + bonus (l - 1)
                  | otherwise -> do
                    vl <- readArray arr (l - 1, r)
                    vr <- readArray arr (l, r + 1)
                    return $ max (vl + bonus (l - 1)) (vr + bonus (r + 1))

            writeArray arr (l, r) v

        return arr

  let !_ = traceMat2D dp (1, n) (1, n)
