...
main = do
  ...
  -- let cache = powerCache
  let !x = factMod n modulus
  let !d1 = factMod r modulus
  let !d2 = factMod (n - r) modulus

  let x1 = mulMod x (invModF d1 modulus) modulus
  let x2 = mulMod x1 (invModF d2 modulus) modulus
