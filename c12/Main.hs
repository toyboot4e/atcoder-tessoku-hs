...
main = do
  ...
  scoreSpans <- replicateM m ((\[l, r] -> (l, r)) <$> getLineIntList)

  let score l r = length $ filter (\(l', r') -> l <= l' && r' <= r) scoreSpans

  -- let rightmosts = combinations k [1 .. n]
  -- let spans = map f rightmosts
  --     f :: [Int] -> [(Int, Int)]
  --     f rights = snd $ mapAccumL g (1 :: Int) rights
  --     g :: Int -> Int -> (Int, (Int, Int))
  --     g left right = (succ right, (left, right))
  -- let result = maximum $ map (sum . map (uncurry score)) spans

  let dp = runSTUArray $ do
        -- arr ! (nDoneSplit, rightmost)
        arr <- newArray ((0, 0), (k, n)) (0 :: Int)

        forM_ [1 .. (n - pred k)] $ \r -> do
          writeArray arr (1, r) $ score 1 r

        forM_ [2 .. k] $ \nSplit -> do
          -- insert new span
          forM_ [nSplit .. n - (k - nSplit)] $ \r -> do
            -- REMAKR: strict range (`[pred nSplit .. pred r]`)
            v <- maximum <$> mapM (\lastRight -> (score (lastRight + 1) r +) <$> readArray arr (nSplit - 1, lastRight) ) [pred nSplit .. pred r]
            writeArray arr (nSplit, r) v
