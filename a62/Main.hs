...
main = do
  ...
  putStrLn $ if isConnected n links then "The graph is connected." else "The graph is not connected."

-- NOTE: IS.size takes O(n) time
isConnected :: Int -> Array Int [Int] -> Bool
isConnected expected links = IS.size (loop IS.empty 1) == expected
  where
    loop explored i
      | isExploredI = explored
      | otherwise =
        let explored' = IS.insert i explored
         in
          foldl' loop explored' (links ! i)
      where
        isExploredI = IS.member i explored
