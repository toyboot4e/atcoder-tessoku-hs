...
main = do
  ...
  ranges <- sortBy (comparing (\(a, b) -> a + 86400 * b)) <$> replicateM nInput ((\[a, b] -> (a, b)) <$> getLineIntList)
  let !_ = traceShow ranges ()

  let nMovies = fst $ foldl' step (0, 0) ranges
      step s@(n, constraint) (l, r)
        | constraint <= l = (succ n, r)
        | otherwise = s
